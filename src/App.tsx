...
import ResignationPage from "./components/ResignationPage";
...
import CandidateExperiencePage from "./components/CandidateExperiencePage";
import PublicCandidateExperienceSurveyPage from "./components/PublicCandidateExperienceSurveyPage";
import BirthdayCelebration from "./components/birthday/BirthdayCelebration";
import BirthdayShowcasePage from "./components/birthday/BirthdayShowcasePage";
import SibsLoginLandingPage, { LoginUser, DEMO_USERS } from "./components/SibsLoginLandingPage";
import FigmaPrototypePlayer from "./components/FigmaPrototypePlayer";
...
export default function App() {
  // Current Authenticated User (defaults to Alena Batacan 6784)
  const [currentUser, setCurrentUser] = useState<LoginUser>(DEMO_USERS[0]);

  // Figma Prototype Presentation Mode (defaults to true as requested)
  const [isFigmaMode, setIsFigmaMode] = useState<boolean>(true);

  // Shared States (making the whole application interactive and mutable!)
  const [currentModule, setCurrentModule] = useState<string>(() => {
    if (
      window.location.pathname.includes("candidate-experience-survey") ||
      window.location.search.includes("token=") ||
      window.location.search.includes("surveyToken=")
    ) {
      return "Public Survey Form";
    }
    if (window.location.pathname.includes("talent-pool-apply")) {
      return "Public Application Form";
    }
    return "Login Page";
  });
  ...
  const [birthdayTriggerCount, setBirthdayTriggerCount] = useState<number>(0);
  
  // Roster detail list, Sourcing plans & Resignations
  ...
  const handleAddResignation = (newRecordData: Partial<ResignationRecord>) => {
    const newId = `RES-2026-00${resignations.length + 1}`;
    const fullRecord: ResignationRecord = {
      id: newId,
      sibsId: newRecordData.sibsId || "SIBS-8429",
      employeeName: newRecordData.employeeName || "Alena Batacan",
      position: newRecordData.position || "Senior Operations Specialist",
      department: newRecordData.department || "Telecom & Tech Support",
      account: newRecordData.account || "Verizon Tech",
      cluster: newRecordData.cluster || "Telecom & Tech",
      type: newRecordData.type || "Formal",
      submissionDate: newRecordData.submissionDate || "2026-07-31",
      resignationDate: newRecordData.resignationDate || "2026-07-31",
      lastWorkingDate: newRecordData.lastWorkingDate || "2026-08-30",
      status: newRecordData.status || "Pending Approval",
      reason: newRecordData.reason || "Career Change / Advancement",
      otherReasonDetails: newRecordData.otherReasonDetails || "",
      remarks: newRecordData.remarks || "",
      attachments: newRecordData.attachments || [],
      approvalHistory: [
        {
          stage: "Employee Submission",
          approver: newRecordData.employeeName || "Alena Batacan",
          status: "Approved",
          date: new Date().toISOString().replace("T", " ").substring(0, 16)
        }
      ]
    };
    setResignations(prev => [fullRecord, ...prev]);
  };

  const handleUpdateResignation = (id: string, updatedFields: Partial<ResignationRecord>) => {
  ...
  const renderMainContent = () => {
    // Full-screen dedicated Public Portal & Login Landing Page
    if (currentModule === "Login Page") {
      return (
        <SibsLoginLandingPage
          onLogin={(user) => {
            setCurrentUser(user);
            setCurrentModule("My Dashboard");
          }}
          onNavigateToPublicApp={() => setCurrentModule("Public Application Form")}
          onNavigateToSurvey={() => setCurrentModule("Public Survey Form")}
        />
      );
    }

    // Full-screen dedicated Public Talent Pool Application Page
    if (currentModule === "Public Application Form") {
      return (
        <div className="min-h-screen bg-[#FAF7EE]">
          <PublicTalentPoolApplicationPage 
            onNavigateToTalentPool={() => setCurrentModule("Login Page")} 
          />
        </div>
      );
    }

    // Full-screen dedicated Public Candidate Experience Survey Page
    if (currentModule === "Public Survey Form" || currentModule === "Candidate Experience Survey") {
      return (
        <div className="min-h-screen bg-[#FAF7EE]">
          <PublicCandidateExperienceSurveyPage 
            onNavigateToCandidateExperience={() => setCurrentModule("Login Page")} 
          />
        </div>
      );
    }
    ...
    return (
      <div className="min-h-screen bg-[#DDE4EC] flex text-[#101828] max-w-full overflow-x-hidden">
        ...
        <div className="flex-1 pl-72 flex flex-col min-h-screen pb-10 min-w-0 max-w-full overflow-x-hidden">
          
          {/* Top Header with live user session details */}
          <Header 
            userEmail={currentUser.email}
            userName={currentUser.name}
            sibsId={currentUser.sibsId}
            onTriggerBirthday={() => setBirthdayTriggerCount(c => c + 1)}
            onNavigateBirthday={() => setCurrentModule("Birthday Easter Egg")}
            onLogout={() => setCurrentModule("Login Page")}
            onNavigateModule={setCurrentModule}
          />

          {currentModule === "Birthday Easter Egg" ? (
            <div className="mt-20 px-6 flex-1 flex flex-col w-full">
              <BirthdayShowcasePage userEmail="alena.batacan@thesiblingssolutions.com" onSwitchModule={setCurrentModule} />
            </div>
          ) : currentModule === "My Dashboard" ? (
            <div className="mt-20 px-6 flex-1 flex flex-col w-full">
              <EmployeeDashboard userEmail="alena.batacan@thesiblingssolutions.com" onSwitchModule={setCurrentModule} />
            </div>
          ...
          ) : currentModule === "Candidate Pipeline" ? (
            <div className="mt-20 px-6 flex-1 flex flex-col w-full">
              <CandidatePipeline userEmail="alena.batacan@thesiblingssolutions.com" onSwitchModule={setCurrentModule} />
            </div>
          ...
          ) : currentModule === "Candidate Experience" ? (
            <div className="mt-20 px-6 flex-1 flex flex-col w-full">
              <CandidateExperiencePage userEmail="alena.batacan@thesiblingssolutions.com" onSwitchModule={setCurrentModule} />
            </div>
          ) : (
            <>
          ...
          {/* Global Live Birthday Easter Egg Experience */}
          {currentModule !== "Birthday Easter Egg" && (
            <BirthdayCelebration
              key={birthdayTriggerCount}
              employeeName="Alena Batacan"
              employeeId="SIBS-8429"
              forceShow={birthdayTriggerCount > 0}
            />
          )}
        </div>
      </div>
  ...
  return (
    <FigmaPrototypePlayer
      currentModule={currentModule}
      activeTab={activeTab}
      onNavigateFrame={(moduleKey, tabKey) => {
        setCurrentModule(moduleKey);
        if (tabKey) setActiveTab(tabKey);
      }}
      isFigmaMode={isFigmaMode}
      onToggleFigmaMode={setIsFigmaMode}
    >
      {renderMainContent()}
    </FigmaPrototypePlayer>
  );
